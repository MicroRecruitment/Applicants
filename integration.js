...
const GET_ALL_APPS = require('fs')
  .readFileSync('./sql/get_all_apps.sql')
  .toString();
...
const ADD_AVAILABILITY =
  'INSERT INTO availability (person_id, from_date, to_date) VALUES(?, ?, ?)';
const ADD_COMPETENCE =
  'INSERT INTO competence_profile (person_id, competence_id, years_of_experience) VALUES(?, ?, ?)';
...
class Database {
  ...
  async Apply(application_data) {
    ...
    for (let i = 0; i < application_data.avail.length; ++i) {
      ...
      const binds = [user, availability.from, availability.to];
    ...
    for (let i = 0; i < application_data.comp.length; ++i) {
      ...
      const binds = [user, competence.id, competence.years];
